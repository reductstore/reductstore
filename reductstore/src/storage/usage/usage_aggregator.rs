...
/// Owns the background task that flushes usage statistics every
/// [`USAGE_FLUSH_INTERVAL`] and once more when it is dropped.
///
/// The worker is shut down the same way the audit aggregator's worker is: by
/// closing a channel. Dropping the aggregator drops the sender, the worker's
/// `recv()` resolves, and it flushes the partial interval before exiting. This
/// mirrors `audit_logger`, so the usage logger needs no explicit stop plumbing
/// in `StateKeeper`/the launcher.
pub(crate) struct UsageEventAggregator {
    // Dropping this closes the channel, signalling the worker to flush the
    // partial interval and exit.
...
impl UsageEventAggregator {
    pub(crate) fn new(
        ...
        tokio::spawn(Self::run_worker(shutdown_rx, sink, storage, counters));
        Self {
...
#[async_trait]
impl LogSystemEvent for UsageEventAggregator {
    /// Usage events are generated by the internal timer, not pushed in by
    /// callers, so receiving an event is a no-op. The type implements
    /// [`LogSystemEvent`] only so it can be stored as a boxed system logger in
    /// `Components`, the same way the audit logger is.
