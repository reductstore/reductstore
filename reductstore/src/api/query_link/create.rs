...
pub(super) async fn create(
    ...
    //     .map_err(|e| -> ReductError { internal_server_error!(&format!("Failed to encrypt query: {}", e)) })?;
    //
    // // encode to base64
    // let ct_b64 = base64::engine::general_purpose::STANDARD.encode(&ct);
    // let salt_b64 = base64::engine::general_purpose::STANDARD.encode(&salt);
    //
    // let link = format!("/api/v1/query_links?ct={}&s={}&i={}", ct_b64, salt_b64, token.name.as_str());
...
fn derive_key_from_secret(secret: &[u8], salt: &[u8]) -> [u8; 32] {
    ...
    let binding = base64::engine::general_purpose::STANDARD.encode(salt);
    let salt = Salt::from_b64(&binding).unwrap();
    // Use the lower-level API to get raw bytes:
    let mut key = [0u8; 32];
    argon2
        .hash_password_into(secret, salt.as_str().as_bytes(), &mut key)
        .unwrap();
    key
}
