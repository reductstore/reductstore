...
TEST_CASE("FullAccess should check full access flag of token") {
  ...
  REQUIRE(FullAccess().Validate({permissions, Error::kOk}) == Error::kOk);

  permissions.set_full_access(false);
  REQUIRE(FullAccess().Validate({permissions, Error::kOk}) == Error::Forbidden("Token doesn't have full access"));
}

TEST_CASE("ReadAccess should work for full access flag and certain bucket") {
  Token::Permissions permissions;
  permissions.set_full_access(true);
  REQUIRE(ReadAccess("bucket-1").Validate({permissions, Error::kOk}) == Error::kOk);
...
TEST_CASE("WriteAccess should work for full access flag and certain bucket") {
  Token::Permissions permissions;
  permissions.set_full_access(true);
  REQUIRE(WriteAccess("bucket-1").Validate({permissions, Error::kOk}) == Error::kOk);
