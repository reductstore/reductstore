...
def _write_records(base_url, session, bucket, entry_0, entry_1):
    ...
    resp = session.post(
        f"{base_url}/b/{bucket}/{entry_1}?ts=1010",
        data="1",
...
def test_update_labels_in_batch(base_url, session, bucket, entry_root):
    ...
    resp_entry_0 = session.get(f"{base_url}/b/{bucket}/{entry_0}?ts=1000")
    ...
    resp_entry_1 = session.get(f"{base_url}/b/{bucket}/{entry_1}?ts=1010")
