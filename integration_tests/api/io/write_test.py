...
def test_write_batched_orders_by_entry_then_time(base_url, session, bucket, entry_root):
    """
    ...
    resp_entry_0 = session.get(f"{base_url}/b/{bucket}/{entry_0}?ts=1010")
    assert resp_entry_0.status_code == 200
    assert resp_entry_0.content == b"AA"
    assert resp_entry_0.headers["content-type"] == "text/plain"

    resp_entry_1 = session.get(f"{base_url}/b/{bucket}/{entry_1}?ts=1000")
