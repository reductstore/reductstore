...
2) Start ReductStore with Zenoh enabled:
   `RS_DATA_PATH=~/data-test RS_ZENOH_ENABLED=1 RS_ZENOH_CONFIG="mode=client;connect=tcp/127.0.0.1:7447" RS_ZENOH_SUB_KEYEXPRS="**" RS_ZENOH_QUERY_KEYEXPRS="**" cargo run -p reductstore --features "fs-backend web-console zenoh-api"`
3) Run tests:
